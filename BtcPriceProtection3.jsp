...
<%@page import="java.io.PrintWriter, java.sql.*"%>
 ...
 double btcTradeAmount = Double.parseDouble(request.getParameter("btcLockAmount"));
  ...
  try {
     ...
     if(btcTradeAmount!=0){
//	     String sql="INSERT INTO Protect (protect_id, name, password, unmatched) values (0, '"+userName+"', '"+ password+"', "+btcTradeAmount+")";
	     String sql="INSERT INTO Protect (protect_id, name, password, unmatched) values (0, ?, ?, ?);";
...
//-------------------------
	    ...
	    // stmt.executeUpdate(sql);
	     //--------------------------------------
			...
			if(rs.next()){
				protect_id2=rs.getInt(1);
			}     
	     
//-----------------------------	     
	     sql="select btc_holdings from users where email='"+userName+"';";
	     ...
	     rs.next();
	      ...
	      if(newBtcHoldings>=0){
	           sql="UPDATE users set btc_holdings="+newBtcHoldings+" where email='"+userName+"';";
	           stmt.executeUpdate(sql);
	      }
	      else{
	    ...
	    // conn.close();
	     second.MatchingEngine.match(out2); 
		    ...
		    sql="select unmatched, matched from Protect where protect_id="+protect_id2+" ;";			 
		    rs = stmt.executeQuery(sql);			 
		    if(rs.next()){
		    	double unmatched = rs.getDouble("unmatched");
		    	out2.println("<h2>Protect "+unmatched+" is UNMATCHED</h2>");
		    	double matched = rs.getDouble("matched");
		    	out2.println("<h2>Protect "+matched+" is MATCHED</h2>");
		    }
 ...
 /*	  out.println("<div style='background-color: #dddddd; margin-left:auto; margin-right: auto; width: 200px; padding: 15px;'>");
		   	        ...
		   	        // out.println(" <br/>");
		        		 ...
		        		 */
		        		 ...
		        		 out.println("<form action='DeleteUnMatched' method='POST'  >");
		        		 out.println("Delete Pending Orders");
		        		// out.println("<input type='hidden' id='name' name='name' value='"+userName+"'>");
		         		// out.println("<input type='hidden' id='password' name='password' value='"+password+"'>");
		         		out.println("<input type='hidden' id='btcLockAmount' name='btcLockAmount' value='"+btcTradeAmount+"'>");
		        		 out.println("<input type=Submit value='Delete' class='btn btn-hero'>");   	            
		        		 out.println("</form>");
 %>		        		 
    ...
    %>
     <div style='display:inline-block;width:100%;'><div style='float: left;'>
     <%
    
     out.println("<center><h4>Protect</h4>");
     out.println("<table style='margin-left: 25px'>");
     ...
     while(rs.next()){
       ...
       //  name = rs.getString("name");
        ...
        out.println("<tr><td>"+protect_id+"</td><td>"+userName+"</td><td>"+unmatched+"</td><td>"+matched+"</td></tr>");
     }
     ...
     sql="Select * from Trading where name= ? ORDER BY trade_id DESC;";
    ...
    //  rs = stmt.executeQuery(sql);
     ...
     while(rs.next()){
        ...
        double unmatched = rs.getDouble("unmatched");
        double matched = rs.getDouble("matched");
        out.println("<tr><td>"+trade_id+"</td><td>"+userName+"</td><td>"+unmatched+"</td><td>"+matched+"</td></tr>");
     }
     out.println("</table></center>");
     out.println("</div>");
     rs.close();
