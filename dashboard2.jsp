...
<!DOCTYPE html>
<%@page
	import="java.io.*, java.sql.*,java.net.URL,javax.json.*,javax.servlet.*,javax.servlet.http.* "%>
	...
	<tr>
		<td style='margin: 25px;'>
			<div style='float: right;'>
				...
				<h5 align="center"><%=email%>
				</h5>
				<!--Display values-->
				<center>
					<table class='fixed_header' border='1' style='font-size: 12px;'>
						<tr>
							<td>Current BTC Balance</td>
							<td style='padding-left: 20px;'><b><%=bitCoin%></b></td>
						</tr>
						<tr>
							<td>Pending BTC Deposit</td>
							<td style='padding-left: 20px;'><b><%=PendingBTCDep%></b></td>
						</tr>
						<tr>
							<td>Pending BTC Withdrawal</td>
							<td style='padding-left: 20px;'><b><%=PendingBTCWithdrawal%></b>
							</td>
						</tr>
						<tr>
							<td>Current LOCK Balance</td>
							<td style='padding-left: 20px;'><%=lotCoin%></td>
						</tr>
						<tr>
							<td>Pending LOCK Withdrawal</td>
							<td style='padding-left: 20px;'><%=PendingLOCKWith%></td>
						</tr>
					</table>
				</center>
				<br />
				...
				<div style='text-align: center; margin-bottom: 50;'>
					...
					<h5>
						...
						<%=lockCoinValue%></h5>
					<h5>
						LOCKCOIN LEFT AT THIS PRICE:<%=coinsAvailableInTier%></h5>
				</div>
			</div>
		</td>
			...
			<div style='float: left; margin: auto; width: 20%;'>
				<center>
					...
					<div style='text-align: center; margin-top: 100px;'>
						<form method='POST' action='Trading2.jsp'
							onsubmit='myButton.disabled = true; return true;'>
							<h6>Amount you want to Autotrade</h6>
							<input type='text' id='autoTrade' name='autoTrade' value=''>
							<input type='submit' name='myButton' value='   Trade   '
								class='btn btn-hero'>
						</form>
					</div>
					<!-- =======================Lock Bitcoin======================================================-->
					<div style='text-align: center; margin-top: 10px;'>
						<form method='POST' action='MatchBTC'
							onsubmit='myButton.disabled = true; return true;'>
							<input type='submit' name='myButton'
								value='   Buy BTC Position  ' class='btn btn-hero'>
						</form>
					</div>
					<!-- =======================Transaction Information======================================================-->
					<div style='text-align: center; margin-top: 10px;'>
						<!--//<h4>Buy Lock Coin</h4>	-->
						<form method='POST' action='DispalyTransactions'
							onsubmit='myButton.disabled = true; return true;'>
							<!-- //<h4>Amount of LockCoin to Buy </h4><input type='text' id='buyLockAmount' name='buyLockAmount' value=''>-->
							<input type='submit' name='myButton'
								value='Transaction Information' class='btn btn-hero'>
						</form>
					</div>
				</center>
			</div>
		...
		<td style='padding: 25px;'>
			...
			<form action='BtcPriceProtection3.jsp' method='post'
				...
				<fieldset>
					...
					Amount of BTC you want to lock: <br> <input type='text'
						name='btcLockAmount' value=''> </br>
					</br> <input type='submit' name='myButton' value='Submit'
						class='btn btn-hero'></br>
				</fieldset>
			</form> <!----------------------------end price protect------------------------------>
		</td>
		<td style='margin: 25px;'>
			<!----------------------------trade----------------------------------    -->
			<form action='TradeInfo2.jsp' method='post'
				onsubmit='myButton.disabled = true; return true;'>
				<fieldset>
					<legend>Give Price Protection:</legend>
					Amount of BTC you want to trade:<br> <input type='text'
						name='btcTradeAmount' value=''></br>
					</br> <input type='submit' name='myButton' value='Submit'
						class='btn btn-hero'></br>
				</fieldset>
			</form> <!----------------------------end trade---------------------------- -->
		...
		<td style='margin: 25px;'>
			<!-------------deposit btc---------------------------------->
			<form action='BTCDEP' method='POST'
				onsubmit='myButton.disabled = true; return true;'>
				<input type='text' id='BTC' name='BTC' value=''> <input
					type='submit' name='myButton' value='Deposit/Withdraw BTC Amount'
					class='btn btn-hero'>
			</form>
		</td>
		<!---------------end deposit-------------------------------  -->
		<!-------------- btc send---------------------------------->
		<td style='margin: 25px;'>
			<form action='BTCSendUsd' method='POST'
				onsubmit='myButton.disabled = true; return true;'>
				<input type='text' id='BTC' name='BTC' value=''> <input
					type='submit' name='myButton'
					value='Send US Dollars Payments in BTC' class='btn btn-hero'>
			</form>
		</td>
		<!---------------end btc send------------------------------- -->
	</tr>
...
<div
	style='float: left; width: 50%; clear: right; border-top: 1px solid gray; margin-top: 20px; padding-top: 20px;'>
	<center>
		<h3>Buy Lock Coin</h3>
		<form method='POST' action='BuyLock'
			onsubmit='myButton.disabled = true; return true;'>
...
<div
	style='float: right; width: 50%; border-top: 1px solid gray; margin-top: 20px; padding-top: 20px;'>
	<!--==============sell lock coin section==============================================================-->
	<center>
		<h3>Sell Lock Coin</h3>
		<form method='POST' action='SellLock'
			onsubmit='myButton.disabled = true; return true;'>

			<h4>Amount of LockCoin to Sell</h4>
			<input type='text' id='sellLockAmount' name='sellLockAmount' value=''>
			<input type='submit' name='myButton' value='Sell Lock Coin'
				class='btn btn-hero'>
		</form>
	</center>
	<br />
	...
	<center>
		...
		<form method='POST' action='Refer'
			onsubmit='myButton.disabled = true; return true;'>
			Referral:<INPUT class='form-textbox' NAME='referer_email' SIZE=50
				VALUE=' Enter email of person who introduced you. '> <input
				type='submit' name='myButton' value='Send Referral'
				class='btn btn-hero'>
		</form>
	</center>
	<br />
...
<div style='text-align: center;'>
	<form method='POST' action='/second/index.jsp'
		onsubmit='myButton.disabled = true; return true;'>
...
<%!private double getLockCoinPrice(IntReturner ir, DoubleReturner drCoinsAvailableInTier, Statement stmt)
			throws SQLException {
