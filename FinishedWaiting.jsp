...
<%
int escrow_id = Integer.parseInt(request.getParameter("escrowId"));
...
try {
   ...
   // Execute SQL query
    ...
    String sql = "SELECT * FROM escrow where escrow_id = "+escrow_id+" ;" ;
    rs = stmt.executeQuery(sql);
    if(rs.next()){
    	bitCoinPrice = rs.getDouble("btc_price_at_lock");
    	lockCoinPrice = rs.getDouble("lockc_price_at_lock");
    	amtToLock = rs.getDouble("btc_escrow");
...
%>
 <h5>Valuation of Price and Market Cap.</h5>
 <table style="background-color: #aaaadd; border: 1px solid black;  ">
   <tr>
         <th colspan="2" >Lock Coin Valuation in USD</th>	
   </tr>
   <tr>
         <td>Freeze BTC Opening</td><td ><input type="checkbox" name="freezeOpen" id="freezeOpen" value="checked" /></td>
   </tr>  
   <tr>
         <td>Opening BTC price</td><td ><input type="text" name="c57" id="c57" value=<%= bitCoinPrice %> /></td>
   </tr>  
 </table> 
  <br/>
 <table style="background-color: #aaddaa; border: 1px solid black;  " >
   <tr>
         <td  >Amount of BTC to LOCK</td><td style="background-color: #aaddaa; "><input type="text" name="e100" id=e100 value="<%= amtToLock %>" style="background-color: #ffffff;"/></td>	
   </tr>
   <tr>
         <td>Current  BTC Price to Open</td><td style="background-color: #aaddaa; ">	<input type="text" name="e101" id="e101" value="<%= bitCoinPrice %>" /></td>
   </tr>
   <tr>      
         <td>Current LOCK Price to Open</td><td style="background-color: #aaddaa; ">	<input type="text" name="e102" id="e102" value="<%= lockCoinPrice %>" /></td>
   </tr>  
 </table> 
 ...
 <br/> 
 <table style="background-color: #ddaaaa;"   >
   <tr>
         <th colspan="2" >Coin Lock Contract </th >	
   </tr>
   <tr>
         <td>Amount of leverage</td><td ><input type="text" name="e107" id="e107" value="4" style="background-color: #ffffff;" /></td>
        
   </tr>  
 </table>
   <br/>
  <table style="background-color: #aaaaaa;"   >
   <tr><th>Protect BTC Balance</th><th>Leverage BTC Balance</th></tr>
   <tr><td><input type="text" name=d110 id="d110" value="" /></td><td><input type="text" name="e110" id="e110" value="" /></td></tr>	
 </table>
    <br/>
  <table style="background-color: #afaaaa;"    >
 ...
 <table style="background-color: #aaaadd; border: 1px solid black;  ">
   ...
   <tr>
         <td>Contract Gain/Loss</td><td >	<input type="text" name="e138" id="e138" value="" /></td>
   </tr>  
   <tr>
...
<input type="button" onclick="getLocPrice()" value="Calculate"/> 
 </form>
      ...
      document.getElementById("data").innerHTML = time + 
		   ...
		   "<br />" + lockCoinValue ; 
		     ...
		     if(freezeOpen.checked != true){
		    	 c57.value = (json["bpi"]["USD"]["rate"]).replace(/,/g, '');
		     }
		        
		     if(freezeOpen.checked != true){	     
		           e101.value=(json["bpi"]["USD"]["rate"]).replace(/,/g, '');
		     }
		     c57.value=<%= bitCoinPrice %>;
		     e101.value=<%= bitCoinPrice %>;
		     e100.value=<%= amtToLock %>;
		     e107.value=4;
		    e119.value =locPrice;
...
//ask allan            e125.value=(H183.value)*e100.value;
        //    e125.value=(c57.value/e118.value)*e100.value;
         if(freezeOpen.checked == true){
            ...
            e125.value=(c57.value/e118.value);
