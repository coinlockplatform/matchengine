...
//import second.Protect;
//import second.Trade;

/**
 * Servlet implementation class ExitMatchProtect
 */
public class ExitMatchProtect extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ExitMatchProtect() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        double leverageAmountX3=leverageAmount*3;
		...
		//--------------------------------------------------------------------------------------------------------------	
		  try {
		     ...
		     for(Protect pro: protectList){
		    	 if(pro.unmatched>=leverageAmountX3){
		    		  sql="UPDATE Protect set unmatched="+(pro.unmatched-leverageAmountX3)+", matched="+ (pro.matched+leverageAmountX3)+" where protect_id="+pro.protect_id+";";
		                stmt.executeUpdate(sql);
	 	                sql="INSERT INTO matched ( match_id, protect_email, trade_email, protect_id, trade_id, 	trade_amount_matched, protect_amount_matched, time_stamp, bitcion_price_at_match ) values (0, '"+pro.name+"', '"+trade_email+"', "+pro.protect_id+", "+trade_id+", "+leverageAmount+", "+(leverageAmount*3)+", NULL, "+bitCoinValue+"  );";
		               stmt.executeUpdate(sql);
		               sql = "SELECT * FROM users where email = '"+ protect_email + "' ;" ;
						rs = stmt.executeQuery(sql);
						// Extract data from result set
		               ...
		               sql = "UPDATE users " +
								"SET btc_holdings = "+(btc_holdings+protectAmount)+" "+
								"  where email = '"+ protect_email + "' ;" ;
						stmt.executeUpdate(sql);
						 sql = "UPDATE matched " +
									"SET finished = 1 "+
		     ...
		     if(matched){
		    	 ...
		    	 sql="select * from unmatchedexitprotect where match_id="+match_id+" ;";
		    	 rs = stmt.executeQuery(sql);
		    	 if(!rs.next()) {
		    	    sql="INSERT INTO unmatchedexitprotect ( id, email, amount, match_id, trade_side ) values ( 0, '"+protect_email+"', "+protectAmount+", "+match_id+", "+leverageAmount+");";
		    	    stmt.executeUpdate(sql);
